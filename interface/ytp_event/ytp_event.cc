...
void YtpEvent::Handle(utils::ItpMsg &msg) {
...
void YtpEvent::OnDepthMarketDataHandle(utils::ItpMsg &msg) {
  ipc::message message;
  message.ParseFromString(msg.pb_msg);
  auto &itp_msg = message.itp_msg();

  auto deepdata = reinterpret_cast<YtpMarketDataStruct *>(itp_msg.address());
...
void YtpEvent::OnRspUserLoginHandle(utils::ItpMsg &msg) {
  ipc::message message;
  message.ParseFromString(msg.pb_msg);
  auto &itp_msg = message.itp_msg();

  auto rsp_info = reinterpret_cast<YtpLoginLogoutStruct *>(itp_msg.address());
  if (rsp_info != nullptr) {
    ;
  }
...
void YtpEvent::OnRspUserLogoutHandle(utils::ItpMsg &msg) {
  ipc::message message;
  message.ParseFromString(msg.pb_msg);
  auto &itp_msg = message.itp_msg();
...
void YtpEvent::OnRspAllInstrumentInfoHandle(utils::ItpMsg &msg) {
  ipc::message message;
  message.ParseFromString(msg.pb_msg);
  auto &itp_msg = message.itp_msg();

  auto ytpqsi = reinterpret_cast<YtpInstrumentInfo *>(itp_msg.address());

  InstrumentInfo::Info instrument_info;
  instrument_info.exch = ytpqsi->exchange_id;
  instrument_info.is_trading = true;
  instrument_info.tradeuint = ytpqsi->tradeuint;
  instrument_info.ticksize = ytpqsi->ticksize;
  instrument_info.max_limit_order_volume = ytpqsi->buy_volume_max;
  instrument_info.min_limit_order_volume = ytpqsi->buy_volume_min;
  instrument_info.max_market_order_volume = ytpqsi->buy_volume_max;
  instrument_info.min_market_order_volume = ytpqsi->buy_volume_min;

  auto &market_ser = MarketService::GetInstance();
  market_ser.ROLE(InstrumentInfo).BuildInstrumentInfo(ytpqsi->instrument_id, instrument_info);
...
void YtpEvent::SetBlockControl(const std::string &ins, ctpview_market::BlockControl_Command command) {
