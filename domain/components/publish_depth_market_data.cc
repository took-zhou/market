...
void PublishData::OnceFromDataflowSelectRawtick(const PublishControl &p_c, CThostFtdcDepthMarketDataField *p_d) {
  ...
  iter->set_instrument_id(p_d->InstrumentID);
  iter->set_last_price(Max2zero(p_d->LastPrice));
  iter->set_bid_price1(Max2zero(p_d->BidPrice1));
  iter->set_bid_volume1(p_d->BidVolume1);
  iter->set_ask_price1(Max2zero(p_d->AskPrice1));
  iter->set_ask_volume1(p_d->AskVolume1);
  if (kDataLevel_ == 2) {
    iter->set_bid_price2(Max2zero(p_d->BidPrice2));
    iter->set_bid_volume2(p_d->BidVolume2);
    iter->set_ask_price2(Max2zero(p_d->AskPrice2));
    iter->set_ask_volume2(p_d->AskVolume2);
    iter->set_bid_price3(Max2zero(p_d->BidPrice3));
    iter->set_bid_volume3(p_d->BidVolume3);
    iter->set_ask_price3(Max2zero(p_d->AskPrice3));
    iter->set_ask_volume3(p_d->AskVolume3);
    iter->set_bid_price4(Max2zero(p_d->BidPrice4));
    iter->set_bid_volume4(p_d->BidVolume4);
    iter->set_ask_price4(Max2zero(p_d->AskPrice4));
    iter->set_ask_volume4(p_d->AskVolume4);
    iter->set_bid_price5(Max2zero(p_d->BidPrice5));
    iter->set_bid_volume5(p_d->BidVolume5);
    iter->set_ask_price5(Max2zero(p_d->AskPrice5));
    iter->set_ask_volume5(p_d->AskVolume5);
  }
  iter->set_open_price(Max2zero(p_d->OpenPrice));
  iter->set_volume(p_d->Volume);
...
void PublishData::OnceFromDataflowSelectRawtick(const PublishControl &p_c, XTPMD *p_d) {
  ...
  auto iter = tick_data->add_tick_list();
  iter->set_state(strategy_market::TickData_TickState_active);
  iter->set_instrument_id(p_d->ticker);
  iter->set_last_price(Max2zero(p_d->last_price));
  iter->set_bid_price1(Max2zero(p_d->bid[0]));
  iter->set_bid_volume1(p_d->bid_qty[0]);
  iter->set_ask_price1(Max2zero(p_d->ask[0]));
  iter->set_ask_volume1(p_d->ask_qty[0]);
  if (kDataLevel_ == 2) {
    iter->set_bid_price2(Max2zero(p_d->bid[1]));
    iter->set_bid_volume2(p_d->bid_qty[1]);
    iter->set_ask_price2(Max2zero(p_d->ask[1]));
    iter->set_ask_volume2(p_d->ask_qty[1]);
    iter->set_bid_price3(Max2zero(p_d->bid[2]));
    iter->set_bid_volume3(p_d->bid_qty[2]);
    iter->set_ask_price3(Max2zero(p_d->ask[2]));
    iter->set_ask_volume3(p_d->ask_qty[2]);
    iter->set_bid_price4(Max2zero(p_d->bid[3]));
    iter->set_bid_volume4(p_d->bid_qty[3]);
    iter->set_ask_price4(Max2zero(p_d->ask[3]));
    iter->set_ask_volume4(p_d->ask_qty[3]);
    iter->set_bid_price5(Max2zero(p_d->bid[4]));
    iter->set_bid_volume5(p_d->bid_qty[4]);
    iter->set_ask_price5(Max2zero(p_d->ask[4]));
    iter->set_ask_volume5(p_d->ask_qty[4]);
  }
  iter->set_open_price(Max2zero(p_d->open_price));
  iter->set_volume(p_d->qty);
...
void PublishData::OnceFromDataflowSelectRawtick(const PublishControl &p_c, BtpMarketDataStruct *p_d) {
  ...
  iter->set_instrument_id(p_d->instrument_id);
  iter->set_last_price(Max2zero(p_d->last_price));
  iter->set_bid_price1(Max2zero(p_d->bid_price[0]));
  iter->set_bid_volume1(p_d->bid_volume[0]);
  iter->set_ask_price1(Max2zero(p_d->ask_price[0]));
  iter->set_ask_volume1(p_d->ask_volume[0]);
  if (kDataLevel_ == 2) {
    iter->set_bid_price2(Max2zero(p_d->bid_price[1]));
    iter->set_bid_volume2(p_d->bid_volume[1]);
    iter->set_ask_price2(Max2zero(p_d->ask_price[1]));
    iter->set_ask_volume2(p_d->ask_volume[1]);
    iter->set_bid_price3(Max2zero(p_d->bid_price[2]));
    iter->set_bid_volume3(p_d->bid_volume[2]);
    iter->set_ask_price3(Max2zero(p_d->ask_price[2]));
    iter->set_ask_volume3(p_d->ask_volume[2]);
    iter->set_bid_price4(Max2zero(p_d->bid_price[3]));
    iter->set_bid_volume4(p_d->bid_volume[3]);
    iter->set_ask_price4(Max2zero(p_d->ask_price[3]));
    iter->set_ask_volume4(p_d->ask_volume[3]);
    iter->set_bid_price5(Max2zero(p_d->bid_price[4]));
    iter->set_bid_volume5(p_d->bid_volume[4]);
    iter->set_ask_price5(Max2zero(p_d->ask_price[4]));
    iter->set_ask_volume5(p_d->ask_volume[4]);
  }
  // iter->set_open_price(Max2zero(p_d->open_price));
  iter->set_volume(p_d->volume);
