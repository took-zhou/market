...
const char *CtpMarketBaseApi::GetApiVersion() { return _m_pApi->GetApiVersion(); }
...
int CtpMarketBaseApi::Join() { return 0; }

const char *CtpMarketBaseApi::GetTradingDay() { return _m_pApi->GetTradingDay(); }
...
void CtpMarketBaseApi::RegisterNameServer(char *pszNsAddress) { return; }

void CtpMarketBaseApi::RegisterFensUserInfo(CThostFtdcFensUserInfoField *pFensUserInfo) { return; }
...
int CtpMarketBaseApi::SubscribeForQuoteRsp(char *ppInstrumentID[], int nCount) { return 0; }

int CtpMarketBaseApi::UnSubscribeForQuoteRsp(char *ppInstrumentID[], int nCount) { return 0; }
...
MARKET_LOGIN_STATE CtpMarketApi::getMarketLoginState(void) { return login_state; }
...
void CtpMarketApi::runLogInAndLogOutAlg() {
  while (1) {
    if (ROLE(MarketTimeState).output.status == LOGIN_TIME && login_state == LOGOUT_STATE) {
      ...
      this->logout();
    } else if (marketSpi != nullptr && marketSpi->reConnect > 1 && login_state != LOGOUT_STATE) {
