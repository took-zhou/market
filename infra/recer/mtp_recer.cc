...
void MtpMarketSpi::OnRspUserLogin(const MtpLoginLogoutStruct *login_info) {
...
void MtpMarketSpi::OnRspUserLogout(const MtpLoginLogoutStruct *login_info) {
  if (login_info != nullptr) {
    ipc::message req_msg;
    auto send_msg = req_msg.mutable_itp_msg();
    send_msg->set_address(reinterpret_cast<int64_t>(login_info));
...
void MtpMarketSpi::OnDepthMarketData(const MtpMarketDataStruct *market_data) {
#ifdef BENCH_TEST
...
void MtpMarketSpi::OnRspAllInstrumentInfo(MtpInstrumentInfo *ticker_info) {
...
void MtpMarketSpi::OnFrontDisconnected(int reason) { front_disconnected_ = true; }

bool MtpMarketSpi::GetFrontDisconnected(void) { return front_disconnected_; }
